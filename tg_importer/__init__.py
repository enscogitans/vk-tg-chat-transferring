...
def fill_parser(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--input", metavar="PATH", help="File containing telegram messages")
    parser.add_argument("--no-progress-bar", action="store_true")


async def main(parser: argparse.ArgumentParser, args: argparse.Namespace, config: Config, tg_client: TgClient) -> None:
    tg_messages_path = Path(args.input or config.tg_default_export_file)
    ...
    async with tg_client:
        success: bool = await import_messages(
            tg_client, config.tg.timezone, tg_messages,
            config.tg.max_simultaneously_uploaded_files,
